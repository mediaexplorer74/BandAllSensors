...
using AllInOneSensorDemo.UWP.Views;
using GalaSoft.MvvmLight.Ioc;

namespace AllInOneSensorDemo.UWP
{
    /// <summary>
    ...
    sealed partial class App : Application
    {
        ...
        public App()
        {
            ...
            RequestedTheme = SettingsPageViewModel.SettingsService.Instance.AppTheme;

            RegisterServicesOnIocContainer();
        }

        private void RegisterServicesOnIocContainer()
        {
            var bandService = new BandService();
            SimpleIoc.Default.Register<IBandService>(() => bandService);
        }
        ...
        {
            //App.Current.Resources["SystemListAccentHighColor"] = XamlBindingHelper.ConvertValue(typeof(Windows.UI.Color), "#0096FF"); ;
            ...
            do
            {
                var bandService = SimpleIoc.Default.GetInstance<IBandService>();
                connected = await bandService.Init();
                if (connected)
                {
                    // NOP
                }
